...
df = pd.read_csv(r'C:\Users\chrystal212\Desktop\ML_HW5\introML2019F.task5.train.csv',engine='python')
mymap = {'A':1, 'B':2, 'C':3, 'D':4}
df=df.applymap(lambda s: mymap.get(s) if s in mymap else s)
#print(df.iloc[0,:])
test=pd.read_csv(r'C:\Users\chrystal212\Desktop\ML_HW5\introML2019F.task5.test.shuffled.noanswers.csv')
#print(test.keys())
test=test.applymap(lambda s: mymap.get(s) if s in mymap else s)

X_train_NU=df.iloc[:, 0:26]
X_train_NO=df.iloc[:, 26]
...
X_test_NU=test.iloc[:, 0:26]
X_test_NO=test.iloc[:, 26]
...
data.to_csv('HW5.csv',index = True,index_label='ID') 
